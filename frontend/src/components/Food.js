...
// Food Page Component - Professional Violet Theme
const Food = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const foodProducts = response.data.filter(product =>
                    product.category?.toLowerCase().includes('food') ||
                    product.category?.toLowerCase() === 'fast-food' ||
                    product.category?.toLowerCase() === 'bakery' ||
                    product.category?.toLowerCase() === 'groceries' ||
                    product.category?.toLowerCase() === 'beverages' ||
                    product.category?.toLowerCase() === 'meals'
                );
    ...
    const filteredProducts = useMemo(() => {
        let result = products.filter(product => {
            ...
            const matchesSubCategory = !filters.subCategory || product.category?.toLowerCase() === filters.subCategory.toLowerCase().replace(/\s+/g, '-');
            const matchesPrice = product.price >= filters.priceRange[0] && product.price <= filters.priceRange[1];
            const matchesRating = filters.rating === 0 || product.rating >= filters.rating;
            return matchesSearch && matchesSubCategory && matchesPrice && matchesRating;
        });
    ...
    const foodSubCategories = ['Fast Food', 'Bakery', 'Groceries', 'Beverages'];

    const clearFilters = () => {
        setFilters({ subCategory: "", priceRange: [0, 100000], rating: 0 });
        setSearchQuery("");
    };
            ...
            <div className="bg-white border-b border-gray-200 pt-20">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
                    <nav className="text-sm text-gray-500 mb-3">
                        ...
                        <Link to="/shop" className="hover:text-violet-600">Products</Link>
                        <span className="mx-2">›</span>
                    ...
                    <aside className={`${showFilters ? 'block' : 'hidden'} lg:block w-64 flex-shrink-0`}>
                        <div className="bg-white rounded-lg border border-gray-200 p-5 sticky top-36 space-y-6">
                            ...
                            <div className="space-y-3">
                                <h4 className="text-sm font-semibold text-gray-900">Food Categories</h4>
                                <div className="space-y-2 max-h-48 overflow-y-auto">
                                    ...
                                    {foodSubCategories.map(sub => (
                                        <button
                                            key={sub}
                                            className={`w-full text-left text-sm py-1.5 px-2 rounded capitalize transition-colors ${filters.subCategory === sub ? 'bg-violet-50 text-violet-700 font-medium' : 'text-gray-600 hover:bg-gray-50'}`}
                    ...
                    <div className="flex-1">
                        {loading ? (
                            <div className="flex flex-col items-center justify-center py-24">
                                ...
                                <p className="text-gray-500">Loading food products...</p>
                            </div>
                        ) : error ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-24 px-6">
                                <div className="max-w-2xl w-full">
                                    <div className="text-center mb-8">
                                        ...
                                        <h2 className="text-3xl font-bold text-gray-900 mb-2">No Food Products Found</h2>
                                        <p className="text-lg text-gray-600">
                                            We apologize, but we couldn't find any food products matching your current search criteria.
                                        </p>
                                    </div>

                                    <div className="bg-gray-50 rounded-lg p-6 mb-8">
                                        <h3 className="text-lg font-semibold text-gray-900 mb-4">Suggestions to help you find what you're looking for:</h3>
                                        <ul className="text-gray-700 space-y-2">
                                            ...
                                            <li className="flex items-start gap-3">
                                                <div className="w-2 h-2 bg-violet-600 rounded-full mt-2 flex-shrink-0"></div>
                                                <span>Use more general terms (e.g., "bread" instead of "sourdough")</span>
                                            </li>
                                            <li className="flex items-start gap-3">
                                                <div className="w-2 h-2 bg-violet-600 rounded-full mt-2 flex-shrink-0"></div>
                                            ...
                                            <li className="flex items-start gap-3">
                                                <div className="w-2 h-2 bg-violet-600 rounded-full mt-2 flex-shrink-0"></div>
                                                <span>Browse our food categories for inspiration</span>
                                            </li>
                                        </ul>
                                    </div>
                                    ...
                                    <div className="flex flex-col sm:flex-row gap-4 justify-center mb-8">
                                        <Button
                                            ...
                                            className="bg-violet-600 hover:bg-violet-700 text-white px-8 py-3 rounded-lg font-semibold shadow-sm hover:shadow-md transition-all"
                                        >
                                        ...
                                        <Button
                                            ...
                                            className="border-gray-300 hover:border-violet-300 hover:bg-violet-50 text-gray-700 px-8 py-3 rounded-lg font-semibold"
                                        >
                                    ...
                                    <div className="border-t border-gray-200 pt-8">
                                        <h4 className="text-lg font-semibold text-gray-900 mb-4 text-center">Popular Food Searches</h4>
                                        <div className="flex flex-wrap gap-3 justify-center">
                                            {['Pizza', 'Fresh Bread', 'Organic Milk', 'Coffee Beans', 'Pasta', 'Rice', 'Chicken', 'Vegetables'].map((term) => (
                                                <button
                                                    key={term}
                                                    onClick={() => setSearchQuery(term)}
                                                    className="px-4 py-2 bg-white border border-gray-300 hover:border-violet-300 hover:bg-violet-50 text-gray-700 hover:text-violet-700 rounded-lg text-sm font-medium transition-all shadow-sm hover:shadow-md"
                                                >
                                                    {term}
                                                </button>
                                            ))}
                                                    ...
                                                    <div className="mt-auto flex items-center justify-between">
                                                        ...
                                                        <div className="flex gap-2">
                                                            <Button
                                                                ...
                                                                variant="outline"
                                                                onClick={() => isInWishlist(product.id) ? removeFromWishlist(product.id) : addToWishlist(product)}
                                                                className={`h-8 w-8 rounded-lg border-gray-300 ${isInWishlist(product.id) ? 'border-rose-200 bg-rose-50 text-rose-500' : 'hover:border-violet-300 hover:text-violet-600'}`}
                                                            >
                                                                <Heart className={`w-3 h-3 ${isInWishlist(product.id) ? 'fill-current' : ''}`} />
                                                            </Button>
                                                            <Button
                                                                onClick={() => addToCart(product)}
                                                                className="h-8 px-3 bg-violet-600 hover:bg-violet-700 text-white rounded-lg text-sm"
                                                            >
